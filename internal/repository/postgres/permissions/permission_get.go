...
func (r *PermissionsRepository) GetPermission(ctx context.Context, key string) (entity.Permission, bool, error) {

	row := pgEntity.NewPermissionRow().FromEntity(entity.Permission{Key: key})

	if err := r.GetOne(ctx, row, row.ConditionUidEqual()); err != nil {
		log.Printf("repositoryPermissions.GetPermission: failed to get permission %s: %v", key, err)
