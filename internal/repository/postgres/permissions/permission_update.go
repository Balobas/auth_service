...
func (r *PermissionsRepository) UpdatePermission(ctx context.Context, perm entity.Permission) error {
	row := pgEntity.NewPermissionRow().FromEntity(perm)

	if err := r.Update(ctx, row, row.ConditionUidEqual()); err != nil {
		log.Printf("repositoryPermissions.UpdatePermission: failed to update permission (key %s): %v", perm.Key, err)
